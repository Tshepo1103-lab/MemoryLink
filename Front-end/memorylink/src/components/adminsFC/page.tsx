  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_:any, record:any) => (
        <Space size="middle">
          <Button onClick={() => editAdmin(record)}><EditOutlined /></Button>
          <Button onClick={() => deleteAdmin(record.id)}><DeleteOutlined /></Button>
        </Space>
      ),
    },
  ].filter(Boolean);
    ...
    <div style={{margin:'50px'}}>
      ...
      >
        <Form form={form} layout="vertical" initialValues={selectedAdmin}>
          ...
          <Form.Item name="surname" label="Surname" rules={[{ required: true, message: 'Please enter the surname' }]}>
            <Input />
          </Form.Item>
          <Form.Item
            name="email"
