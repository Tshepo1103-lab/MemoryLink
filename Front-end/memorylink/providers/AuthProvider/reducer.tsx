...
export const userReducer = handleActions<IUserStateContext,IUserStateContext> (
    
    {
        ...
        [UserActionEnum.regiserUserRequest]: (state, action) =>({...state, ...action.payload}),
        [UserActionEnum.registerUserSuccess]: (state, action) => ({...state, ...action.payload}),
        [UserActionEnum.registerUserError]: (state, action) => ({ ...state, ...action.payload }),
    },
    INITIAL_STATE
)